...
class GenerateAppKeyTask implements TaskInterface
{
    ...
    public function execute(ProvisionContext $context): TaskResult
    {
        ...
        if ($path === '' || ! is_file($path.'/artisan')) {
            return TaskResult::success('Skipped app key generation (not a Laravel app).');
        }

        $this->shellService->runSystem('php', ['artisan', 'key:generate', '--force'], cwd: $path);

        return TaskResult::success('Application key generated.');
    }
}
