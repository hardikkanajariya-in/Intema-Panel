...
export default function DomainsShow({ domain, sslCertificate, availableCertificates }: DomainsShowProps) {
    const attachSsl = (uuid: string) => {
        if (!uuid) return;
        router.post(`/domains/${domain.uuid}/attach-ssl`, { ssl_certificate_uuid: uuid });
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                ...
                <Card>
                    <CardHeader><CardTitle>SSL Certificate</CardTitle></CardHeader>
                    <CardContent className="space-y-3 text-sm">
                        {sslCertificate ? (
                            <>
                                <div>{sslCertificate.domain_name} — {sslCertificate.status_label}</div>
                                <Button size="sm" variant="outline" onClick={() => router.delete(`/domains/${domain.uuid}/detach-ssl`)}>Detach SSL</Button>
                            </>
                        ) : (
                            <>
                                <p className="text-muted-foreground">No SSL certificate attached.</p>
                                {availableCertificates.length > 0 && (
                                    <Select defaultValue="" onChange={(e) => attachSsl(e.target.value)}>
                                        <option value="">Attach certificate...</option>
                                        {availableCertificates.map((cert) => (
                                            <option key={cert.uuid} value={cert.uuid}>{cert.domain_name}</option>
                                        ))}
                                    </Select>
                                )}
                            </>
